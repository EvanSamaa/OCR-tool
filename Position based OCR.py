...
def emailEvan(to):
	fromMy = 'evan12345pan@yahoo.com' #this did not work for some reason, you have to type it each time
	# to = 'evan12345pan@hotmail.com'
	# to = 'evan.pan@mail.utoronto.ca'
	subj= "Alert"
	date='2/1/2010'
	message_text='Your code is ready'

	msg = "From: %s\nTo: %s\nSubject: %s\nDate: %s\n\n%s" % ( 'evan12345pan@yahoo.com', to, subj, date, message_text )

	username = str('evan12345pan@yahoo.com')  
	password = str('asd321,,,...')  
...
def learn():
	IndexSet1_dataTocollect = ["James Dick Concrete"]
	IndexSet1_nameOfData = ["supplier"]
	IndexSet1_dataIncoordinate = [[2070,752,2333,796] ,[566, 1225, 2327, 1276], [599,932,747,975], [1273,988,1457,1038], [510,995,651,1030], [413, 1171, 975, 1216], [2003, 873, 2144, 917], [1393, 1723, 1578, 1772], [1396, 1786, 1578, 1842], [1195, 1720,1370,1774], [1192, 1783, 1381, 1840]]
	IndexSet1_dataIncoordinateName = ["concrete mix", "location", "measured slump", "specified air%", "measured air%", "admixture", "specified strength", "measured X day strength 1", "measured X day strength 2", "age on concrete 1", "age on concrete 2"]
	...
	for num in IndexSet1_dataIncoordinate:
		IndexSet1_dataType = IndexSet1_dataType + ["non_float"]
...
CleanUp(toDelete)
emailEvan('evan.pan@mail.utoronto.ca')
